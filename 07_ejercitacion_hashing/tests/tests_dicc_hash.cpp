...
TEST(DiccHash, borrar) {
    ...
    ASSERT_FALSE(dicc.definido("cajuan"));
...
TEST(DiccHash, significadoColisiones){ //AGREGADO POR MÍ
    DiccHash<int> dicc;
    ASSERT_EQ(dicc.cantClaves(),0);

    dicc.definir("juanca",40);
    dicc.definir("cajuan", 42);

    ASSERT_TRUE( dicc.definido("juanca") );
    ASSERT_TRUE(dicc.definido("cajuan"));
    ASSERT_EQ(dicc.cantClaves(),2);
    ASSERT_EQ(dicc.significado("juanca"),40);
    ASSERT_EQ(dicc.significado("cajuan"), 42);
...
TEST(DiccHash, colisiones_uniforme) {
    ...
    char delim = '\n';
    std::string archnom = "/home/muripic/Documents/AED_II/08_ejercitacion_hashing/data/uniforme.txt";
...
TEST(DiccHash, colisiones_gaussiana) {
    ...
    char delim = '\n';
    std::string archnom = "/home/muripic/Documents/AED_II/08_ejercitacion_hashing/data/gauss.txt";
...
TEST(DiccHash, colisiones_libretas) {
    ...
    char delim = '\n';
    std::string archnom = "/home/muripic/Documents/AED_II/08_ejercitacion_hashing/data/libretas.txt";
...
TEST(DiccHash, redimensionado) {
    ...
    while(getline(archin,str,delim) && archin.good()) {
        ...
        ASSERT_EQ(dh.significado(str),val);
        val++;
    }
    archin.close();
